...
NAMESPACE="${NAMESPACE:-current-beta}"
...
build_and_push_images() {
    ...
    for svc in "${SERVICES[@]}"; do
        ...
        docker build -t "$IMAGE_REGISTRY/current-$svc:$IMAGE_TAG" -t "$IMAGE_REGISTRY/current-$svc:beta" -f "$PROJECT_ROOT/services/$svc/Dockerfile" "$PROJECT_ROOT"
        log_info "Pushing $svc with tag $IMAGE_TAG..."
        docker push "$IMAGE_REGISTRY/current-$svc:$IMAGE_TAG"
        docker push "$IMAGE_REGISTRY/current-$svc:beta"
...
main() {
    log_section "Current - Beta Deployment"
