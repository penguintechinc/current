...
class Analytics:
    ...
    def get_url_analytics(url_id, days=30):
        ...
        # Get click data
        clicks = db((db.analytics.url_id == url_id) & 
                   (db.analytics.clicked_on >= start_date)).select()
        ...
        analytics_data['countries'] = dict(sorted(
            analytics_data['countries'].items(), 
            key=lambda x: x[1], 
            reverse=True
        )[:10])
        
        analytics_data['cities'] = dict(sorted(
            analytics_data['cities'].items(), 
            key=lambda x: x[1], 
            reverse=True
        )[:10])
        
        analytics_data['top_referers'] = dict(sorted(
            analytics_data['top_referers'].items(), 
            key=lambda x: x[1], 
            reverse=True
        )[:10])
    ...
    def get_top_repeat_visitors(days=30, limit=10):
        ...
        for ip, count in top_ips:
            ...
            latest_click = db(db.analytics.ip_address == ip).select(
                orderby=~db.analytics.clicked_on,
                limitby=(0, 1)
            ).first()
