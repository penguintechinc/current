...
class URLShortener:
    ...
    def create_short_url(
        ...
        if custom_code:
            ...
            if len(custom_code) > MAX_CUSTOM_LENGTH:
                return (
                    None,
                    f"Custom code must be {MAX_CUSTOM_LENGTH} characters or less",
                )
    ...
    def update_short_url(url_id, user_id, **kwargs):
        ...
        if (
            user.role not in ["admin", "contributor"]
            and url_record.created_by != user_id
        ):
    ...
    def get_url_by_short_code(short_code):
        ...
        return (
            db((db.urls.short_code == short_code) & (db.urls.is_active == True))
            .select()
            .first()
        )
    ...
    def get_frontpage_urls():
        ...
        return db(
            (db.urls.is_active == True) & (db.urls.show_on_frontpage == True)
        ).select(orderby=~db.urls.click_count)
