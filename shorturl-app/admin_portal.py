...
class AdminPortal:
    ...
    def setup_routes(self):
        ...
        self.app.router.add_post('/api/certificates/renew', self.handle_renew_certificate)
    ...
    async def handle_register(self, request):
        ...
        if user:
            ...
            return web.json_response({'success': False, 'error': 'User already exists'}, status=400)
    ...
    async def handle_get_urls(self, request):
        ...
        if category_id:
            urls = db((db.urls.is_active == True) & 
                     (db.urls.category_id == category_id)).select()
        else:
        ...
        for url in urls:
            urls_data.append({
                ...
                'created_on': url.created_on.isoformat() if url.created_on else None
            })
    ...
    async def handle_renew_certificate(self, request):
        ...
        if success:
            ...
            return web.json_response({'success': False, 'error': 'Renewal failed'}, status=400)
